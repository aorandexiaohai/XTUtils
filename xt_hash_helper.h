...
struct XTHashCalculator {
	static constexpr unsigned int crc32(const char * str, unsigned int prev_crc = 0xFFFFFFFF)
	{
		return XTHashCalculator<size, idx + 1>::crc32(str, (prev_crc >> 8) ^ crc_table[(prev_crc ^ str[idx]) & 0xFF]);
	}
};

// This is the stop-recursion function
template<int size, class dummy>
struct XTHashCalculator<size, size, dummy> {
	static constexpr unsigned int crc32(const char * str, unsigned int prev_crc = 0xFFFFFFFF)
	{
...
#define XTSTATIC_HASH_STR(x) (XTHashCalculator<sizeof(x)-1>::crc32(x))
